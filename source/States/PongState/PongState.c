...
void PongState::enter(void* owner)
{
	...
	AutomaticPauseManager::setActive(AutomaticPauseManager::getInstance(NULL), false);

	// Get the game ready
	Pong::getReady(Pong::getInstance(NULL), this->stage, false);

	Pong::addEventListener(Pong::getInstance(NULL), ListenerObject::safeCast(this), (EventListener)PongState::onRemoteInSync, kEventPongRemoteInSync);
	Pong::addEventListener(Pong::getInstance(NULL), ListenerObject::safeCast(this), (EventListener)PongState::onRemoteGoneAway, kEventPongRemoteWentAway);
...
{
	Pong::removeEventListener(Pong::getInstance(NULL), ListenerObject::safeCast(this), (EventListener)PongState::onRemoteInSync, kEventPongRemoteInSync);
	Pong::removeEventListener(Pong::getInstance(NULL), ListenerObject::safeCast(this), (EventListener)PongState::onRemoteGoneAway, kEventPongRemoteWentAway);
...
void PongState::processUserInput(const UserInput* userInput)
{
	...
	{
		Pong::processUserInput(Pong::getInstance(NULL), userInput);
	}
...
void PongState::showControls()
{
	...
	if(this->isVersusMode)
	{
		...
		switch(Pong::getPlayerNumber(Pong::getInstance(NULL)))
		{
			case kPlayerOne:
...
void PongState::showExplanation()
{
	...
	Printing::text(I18n::getText(I18n::getInstance(NULL), kStringCommunicationsLabel), 2, y++, NULL);
	...
	Printing::text(I18n::getText(I18n::getInstance(NULL), kStringCollisionsLabel), 26, y++, NULL);
	Printing::text(I18n::getText(I18n::getInstance(NULL), kStringEventsLabel), 26, y++, NULL);
	Printing::text(I18n::getText(I18n::getInstance(NULL), kStringMessagingLabel), 26, y++, NULL);
...
{
	Pong::printScore(Pong::getInstance(NULL));
	PongState::showConnectivityStatus(this);
}
...
void PongState::showConnectivityStatus()
{
	...
	{
		const char* strConnected = I18n::getText(I18n::getInstance(NULL), kStringConnected);
	...
	{
		const char* strNoLink = I18n::getText(I18n::getInstance(NULL), kStringNoLink);
...
{	
	...
	Pong::getReady(Pong::getInstance(NULL), this->stage, true);
